      ...
      <div className="relative mt-1 rounded-md">
        <input
          ...
          className={cn(
            ...
            error
              ? "border-red-700 shadow-none focus-visible:ring-red-700"
              : "focus-visible:ring-blue-700 focus-visible:border-transparent",
            className
          )}
          ref={ref}
          {...props}
        ...
        <div className="absolute inset-y-0 right-0 flex items-center h-[36px]" ref={suffixWrapRef}>
          {suffix || hintText ? (
            <div className={cn("text-gray-500 truncate", cn({ "flex space-x-2 items-center": suffix && hintText }))} style={suffixStyle}>
              <div>{suffix}</div>
              {hintText ? (
